...
import { ArrowRightLeft, RefreshCw, Calculator, Copy, Share2, History } from "lucide-react"
import {
  getNBUCurrentRates,
  convertWithNBURates,
  getCurrencyInfo,
...
export function CurrencyConverterNBUImproved() {
  ...
  const handleConvert = () => {
    ...
    setConvertedAmount(resultFormatted)
  ...
  const copyResult = async () => {
    if (convertedAmount) {
      await navigator.clipboard.writeText(`${fromAmount} ${fromCurrency} = ${convertedAmount} ${toCurrency}`)
      // You could add a toast notification here
    }
  }

  const shareConversion = async () => {
    if (convertedAmount && navigator.share) {
      await navigator.share({
        title: "Currency Conversion",
        text: `${fromAmount} ${fromCurrency} = ${convertedAmount} ${toCurrency} (NBU Rate: ${exchangeRate.toFixed(4)})`,
      })
    }
  }
      ...
      <CardContent>
        <div className="space-y-6">
          ...
          <div className="flex justify-center gap-3">
            ...
            {convertedAmount && (
              <>
                <Button variant="outline" onClick={copyResult} title="Copy result">
                  <Copy className="w-4 h-4" />
                </Button>

                {navigator.share && (
                  <Button variant="outline" onClick={shareConversion} title="Share conversion">
            ...
            <Button variant="outline" onClick={() => setShowHistory(!showHistory)} title="View conversion history">
          ...
          {showHistory && conversionHistory.length > 0 && (
            <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <h4 className="font-medium text-sm text-gray-700 dark:text-gray-300 mb-3">Recent Conversions</h4>
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {conversionHistory.map((conversion) => (
                  <div
                    key={conversion.id}
                    className="flex justify-between items-center text-sm p-2 bg-white dark:bg-gray-600 rounded"
                  >
                    <span>
                      {conversion.fromAmount} {conversion.fromCurrency} → {conversion.result} {conversion.toCurrency}
                    </span>
                    <span className="text-xs text-gray-500">{conversion.timestamp.toLocaleTimeString()}</span>
                  </div>
                ))}
              </div>
            </div>
